...
export default function ReelsView() {
  ...
  return (
    <div className="fixed inset-0 flex bg-black overflow-hidden pt-16 ">
      {/* Close Button */}
      <button
        onClick={() => (window.location.href = "/home")}
        className="flex hover:bg-gray-200 items-center justify-center rounded-full p-2 bg-gray-100 absolute top-20 left-4 z-50 cursor-pointer"
      >
      ...
      <div 
        ...
        className="flex hover:bg-gray-200 items-center justify-center rounded-full p-2 bg-gray-400 absolute top-20 left-[640px] z-50 cursor-pointer"
      >
        {mutedState ? (
      ...
      <div
        ref={containerRef}
        className="flex-1 h-full  overflow-y-auto snap-y snap-mandatory no-scrollbar bg-black"
      >
        {reelsState.map((reel: any, index: number) => (
          <div
            key={index}
            className="h-screen w-full p-2 rounded-lg snap-start flex items-center justify-center relative"
          >
            <div className="w-full rounded-lg max-w-[400px]  h-[600px] relative flex flex-col justify-center">
              <video
                ref={(el) => {
